...
def groups_order_listing(request):
    ...
    if request.method=="GET":
        ...
        # cursor.execute("select order_created_on,group_name,trigger_condition from master_company_group WHERE is_executed=False and group_status=True") 
        # not_executed_group = cursor.fetchall()
        # print('not_executed_group :==',not_executed_group)
        # cursor.execute("select order_created_on,group_name,trigger_condition from master_company_group WHERE is_executed=True and group_status=False") 
        ...
        cursor.execute("Select Count(is_executed) From group_order  Where is_executed = True and user_id = \'"+str(user_id[0])+"\'")
        countexecuteorder = cursor.fetchall()
        print(countexecuteorder[0][0],"executeorder")
        countexecute = countexecuteorder[0][0]

        return render(request,'groups_order_listing.html',{'countopen':countopen,'countexecute':countexecute,'trigger_order':not_executed_group,'executed_group':executed_group})
    else:
