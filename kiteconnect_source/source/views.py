...
def home(request):
    ...
    cursor.execute("Select  count(distinct trigger_id)  From orders  Where is_executed = false  and user_id = "+user_id)
    ...
    cursor.execute("Select count(distinct trigger_id) From orders  Where is_executed = True and user_id = "+user_id)
    ...
    # paginator = Paginator(trigger_name_list, 10)
    # page = request.GET.get('page')
    # print('pageeeeeeeeeeeeeee',page)
    # try:
    #     trigger_name_list = paginator.page(page)
    # except PageNotAnInteger:
    #     trigger_name_list = paginator.page(1)
    # except EmptyPage:
    #     trigger_name_list = paginator.page(paginator.num_pages)


    cursor.execute("SELECT DISTINCT trigger_name, order_created_on from triggers WHERE is_group_trigger=false AND is_active=True AND order_created_or_not=True AND user_id =\'"+user_id+"\'") 
    # cursor.execute("SELECT DISTINCT trigger_name, order_created_on from triggers WHERE is_group_trigger=false AND is_active=True AND user_id =\'"+user_id+"\'") 
    trigger_name_list = cursor.fetchall()
...
# trigger_name
    ...
    # cursor.execute("SELECT DISTINCT trigger_name,order_created_on from triggers WHERE is_group_trigger=false AND is_active=False AND user_id =\'"+user_id+"\'") 
    # executed_trigger_list = cursor.fetchall()
    # paginator = Paginator(executed_trigger_list, 5)
    # page = request.GET.get('page')
    # print("page=================",page)
    # try:
    #     executed_trigger_list = paginator.page(page)
    # except PageNotAnInteger:
    #     executed_trigger_list = paginator.page(1)
    # except EmptyPage:
    #     executed_trigger_list = paginator.page(paginator.num_pages)
...
def criteria_volume(trigger,triggerId,trigger_condition_id,group_id=0):
    ...
    #If it is Volume avg
        ...
        if trigger["trigger_criteria"]["operator"]=="5" and volume_average== Decimal(trigger["trigger_criteria"]["amount"]):
            ...
            print("operator 1 :::")
            # criteria_volume_status_update(triggerId,trigger_condition_id,group_id)
            criteria_volume_status_update(trigger,triggerId,trigger_condition_id,group_id)
...
def criteria_RSI(trigger,triggerId,cursor,trigger_condition_id,group_id=0):
    ...
    # to_date_string=datetime.now().strftime("%Y-%m-%d %H:%M:%S") 
    # total=0.00
    print('criteria_rsi trigger::,',trigger)
    historical_data=None
    # from_date=datetime.now()
    from_date = datetime.datetime.now()
    print("from_date#######$$$$$$$$$$$$$$$$$$$$$$$$+++++++++++++++",from_date)
...
#                     except:
#                         print("except blockllllllllllllllllllllllllllllll")
#                         print("orderid :===",orderid)
...
def delete_order(request):
    ...
    # connection = psycopg2.connect(user="postgres", password="root", host="localhost", port="5432", database="Kite")
    # cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor) 
    # cursor.execute("UPDATE triggers set order_created_or_not = False WHERE trigger_name = \'"+trigger_name+"\'")
    # connection.commit()
...
def save_multiple_trigger(request):   
    if request.method == "POST":
        try:            
            ...
            # user_id = request.session.get('user_id')
            # print("session user_id:========================",user_id)
            x=request.data["trigger"]
            print("trigger in save triggers@@@@@@@@@@@@@@@@@@@@@@@@@@@@@",x)
            ...
            print(type(final_criteria))

            print("final_criteria after replace",final_criteria)
            y=request.data["trigger"]
            print("trigger in save triggers@@@@@@@@@@@@@@@@@@@@@@@@@@@@@",y)
...
def save_multiple_order(request):
    ...
    # trigger_name
        ...
        print("updated sucedss")
        cursor.execute("UPDATE triggers SET order_created_or_not = True WHERE trigger_name=\'"+trigger_name+"\' and user_id="+userid+"")
...
def create_group(request):
    if request.method == "POST":
        ...
        cursor.execute("INSERT INTO master_company_group(group_name,company_list,exchange,user_id,instrument,created_on) VALUES(\'"+group_name+"\',\'"+group_symbol+"\',\'"+group_exchange+"\','"+userid+"',\'"+group_instrument_token+"\',\'"+current_date_time+"\')")
        print("inserted successfull")
        connection.commit()
        return Response({"status":"success", "message": "success"})        
    else:
    ...
    if request.method=="POST": #change to POST
        ...
        for record,instrument in zip(company_list, instrument[0].split(',')):
            if record != "":
                ...
                cursor.execute("INSERT INTO triggers(trigger_name, symbol,criteria , trigger_condition_id,trigger_criteria, exchange_id,user_id,is_group_trigger)VALUES (\'"+trigger_name+"\',\'"+record+"\',\'"+final_criteria+"\',"+trigger_condition_id+",\'"+trigger_criteria+"\',"+exchange_id+","+user_id+",True)") 
                connection.commit()                
            else:
                print("pass")
                pass
...
def delete_group_order(request):
    ...
    if request.method == "POST":
        ...
        # cursor.execute("UPDATE master_company_group SET is_executed=\'"+is_executed+"\' WHERE group_name=\'"+group_name+"\'")
        connection.commit()
        print("deleted")
...
def trigger_list(request):
    ...
    # cursor.execute("select created_on,trigger_name,symbol,criteria,trigger_id from triggers where user_id ="+userid+" and is_group_trigger = False")
    # triggers = cursor.fetchall()
    cursor.execute("select t.created_on,t.trigger_name,t.symbol,t.criteria,t.trigger_id,master_trigger_conditions.trigger_condition  from triggers t INNER JOIN master_trigger_conditions ON t.trigger_condition_id = master_trigger_conditions.trigger_condition_id  where user_id ="+userid+" and is_group_trigger = False and  is_active=True")
    triggers = cursor.fetchall()
    # for i in triggers:
    #     print(i[3]['operator'])
    #     if i[3]['operator'] == '1':
    ...
    print('current_user :',current_user)
    current_user = str(current_user)   
    cursor.execute("Select  count(trigger_name)  From triggers" )
    counttrigger = cursor.fetchall()
    print(counttrigger[0][0],"counttrigger")
    counttrigger=counttrigger[0][0]
    return render(request,'trigger_listing.html',{'triggers':triggers,'current_user':current_user,'counttrigger':counttrigger})
...
def update_triggers(request,trigger_id):
    ...
    if request.method =='POST':
        ...
        # criteria = json.loads(request.data["trigger"]["criteria"])
        # print("json_criteria ;====",type(criteria))
        # print("json_criteria ;====",criteria)
        # json_criteria = json.dumps(request.data["trigger"]["criteria"])
        # print("criteria ;====", json_criteria)
        # print("criteria ;====",type( json_criteria))
        ...
        # exchange_id = '3'
        # trigger_id = '1743'
        cursor.execute("UPDATE triggers set trigger_name = \'"+trigger_name+"\', symbol=\'"+symbol+"\',trigger_condition_id=\'"+trigger_condition_id+"\',trigger_criteria=\'"+trigger_criteria+"\',criteria=\'"+final_criteria+"\',exchange_id=\'"+exchange_id+"\' WHERE trigger_id=\'"+trigger_id+"\'")
        connection.commit()
        ...
        trigger_id = str(trigger_id)
        print("trigger_id ",trigger_id)
        cursor.execute("SELECT t.trigger_id,t.trigger_name,t.symbol,e.exchange,c.trigger_condition,t.trigger_criteria,t.exchange_id,t.trigger_condition_id,t.criteria FROM public.triggers t JOIN public.master_exchange e ON t.exchange_id=e.exchange_id JOIN public.master_trigger_conditions c ON t.trigger_condition_id=c.trigger_condition_id JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' JOIN public.master_exchange eo ON eo.exchange_id=t.exchange_id WHERE t.trigger_id="+trigger_id+" order by t.trigger_id asc")
        triggers = cursor.fetchall()
        print("triggers",triggers)
        cursor.execute("SELECT t.trigger_criteria,t.criteria FROM public.triggers t JOIN public.master_exchange e ON t.exchange_id=e.exchange_id JOIN public.master_trigger_conditions c ON t.trigger_condition_id=c.trigger_condition_id JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' JOIN public.master_exchange eo ON eo.exchange_id=t.exchange_id WHERE t.trigger_id="+trigger_id+" order by t.trigger_id asc")
...
def open_trigger_order_list(request,trigger_name):
    if request.method=="GET":
        ...
        # trigger_order = cursor.fetchall()
        #trigger_name = trigger_order[0][2]
        print('trigger_order[0][2] :"==',trigger_order[0][2])
        # print('trigger_order[0][2] ===============:"==',trigger_order[0][3])
        # print('trigger_order[0][2] ++++++++++++++++++:"==',trigger_order[0][4])
        # print('trigger_order[0][2] //////////////////:"==',trigger_order[0][5])
        # print('trigger_order[0][2] ===============:"==',trigger_order[0][6])
        ...
        cursor.execute("SELECT distinct on(o.order_id)t.created_on, t.trigger_name,t.symbol, master_trigger_conditions.trigger_condition,o.symbol,t.trigger_criteria, master_buy_sell.buy_sell, o.quantity,o.product_type,o.trigger_id,o.order_id, master_product_type.product_type, master_order_type.order_type FROM triggers t JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' JOIN public.orders o ON tcl.trigger_id=o.trigger_id INNER JOIN master_trigger_conditions ON t.trigger_condition_id = master_trigger_conditions.trigger_condition_id INNER JOIN master_product_type ON master_product_type.product_type_id = o.product_type INNER JOIN master_buy_sell ON master_buy_sell.buy_sell_id = o.buy_sell INNER JOIN master_order_type ON master_order_type.order_type_id = o.order_type WHERE o.is_executed='False' AND is_group_trigger=False AND trigger_name =\'"+trigger_name+"\' AND o.user_id=\'"+userid+"\'") 
        orders = cursor.fetchall()
        return render(request,'open_order.html',{'trigger_order':trigger_order,'orders':orders,'trigger_name':trigger_name})
    else:
...
#         cursor.execute("SELECT  t.created_on,t.trigger_name,t.trigger_criteria,c.trigger_condition,t.symbol,o.order_id FROM public.triggers t JOIN public.master_trigger_conditions c ON t.trigger_condition_id=c.trigger_condition_id JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' Join public.master_company_group mcg ON tcl.trigger_id = mcg.trigger_id JOIN public.group_order o ON mcg.id=o.group_id WHERE t.is_group_trigger=true and t.is_active=true and trigger_name  =\'"+trigger_name+"\' order by t.trigger_id ") 
#         trigger_order = cursor.fetchone()

#         cursor.execute("SELECT t.trigger_name,t.trigger_criteria,b.buy_sell,o.amount,mo.order_type,p.product_type,t.symbol,o.order_id,o.is_executed FROM public.triggers t JOIN public.master_exchange e ON t.exchange_id=e.exchange_id JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' Join public.master_company_group mcg ON tcl.trigger_id = mcg.trigger_id JOIN public.group_order o ON mcg.id=o.group_id JOIN public.master_buy_sell b ON o.buy_sell=b.buy_sell_id JOIN public.master_product_type p ON p.product_type_id=o.product_type JOIN public.master_order_type mo ON mo.order_type_id=o.order_type WHERE t.is_active=True and o.is_executed=False and t.trigger_name= \'"+trigger_name+"\'") 
...
def group_open_trigger_order_list(request,trigger_name):
    if request.method=="GET":
        ...
        cursor.execute("SELECT t.trigger_name,t.criteria,b.buy_sell,o.amount,mo.order_type,p.product_type,t.symbol,o.order_id,o.is_executed,t.is_active FROM public.triggers t JOIN public.master_exchange e ON t.exchange_id=e.exchange_id JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' Join public.master_company_group mcg ON tcl.trigger_id = mcg.trigger_id JOIN public.group_order o ON mcg.id=o.group_id JOIN public.master_buy_sell b ON o.buy_sell=b.buy_sell_id JOIN public.master_product_type p ON p.product_type_id=o.product_type JOIN public.master_order_type mo ON mo.order_type_id=o.order_type WHERE t.is_active=True and o.is_executed=False and t.trigger_name= \'"+trigger_name+"\'") 
        open_order = cursor.fetchall()
        print("open_order ",open_order)
...
#Here the delete button in trigger list can delete the data both triger and order at a time
...
def deleteIndividualTriggerOrder(request):
    ...
    if request.method=="POST":
        ...
        cursor.execute("") 
        connection.commit()
        return Response({"status":"success", "message": "successsss"})



#developed by abhishek
#this function used for download all the data in openorder
...
#developed by abhishek
#this function used for download all the data in execute_order_excel
...
def execute_order_excel(request):
   ...
   #take the data through indexing value   
   writer.writerow([
     smart_str(u"Event Create"),
     smart_str(u"Trigger name"),
     smart_str(u"Trigger symbol"),
...
#code developed by abhishek
# Here the function group_order_excel  download all the data in group_order
def group_order_excel(request):
   ...
   return response
#code developed by abhishek
# Here the function group_trigger_excel  download all the data in group_trigger
...
#code developed by abhishek
# Here the function group_executed_trigger_list  download all the data in  group executed trigger list
...
#code developed by abhishek
# Here the function group_executed_order_list  download all the data in  group_executed_order.  
