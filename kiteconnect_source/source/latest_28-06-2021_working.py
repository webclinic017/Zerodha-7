...
def home(request):
    ...
    # open_orders = []
    # for i in rows:
    #     if i[1] not in open_orders:
    #         open_orders.append(i[1])
    #         fruit_dictionary = dict.fromkeys(open_orders, 0)
    #         print("not same :",i[1])
    # print("fruit_dictionary :",fruit_dictionary)
    # final_trigger = []
    # new_trigger = []
    # for key,value in fruit_dictionary.items():
    #     new_trigger.append(key)
    #     print("new_trigger",new_trigger)
    #     fruit_dictionary[key] = 0
    #     if fruit_dictionary[key] <= 1:
    #         final_trigger.append(i)
    #         fruit_dictionary[key] = 1
    # print("fruit_dictionary ======:",fruit_dictionary)
    # print("final_trigger :====:",final_trigger)
    
    dict_result = []
    for row in rows:
        dict_result.append(dict(row))
    # print("dict_result_2 :===========",dict_result)
...
def groups_order_listing(request):
    ...
    cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor) 
    # add group_name
    cursor.execute("SELECT t.created_on, t.trigger_name,t.symbol, master_trigger_conditions.trigger_condition, master_buy_sell.buy_sell,master_product_type.product_type,o.trigger_id,o.order_id,o.amount FROM triggers t JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' JOIN public.group_order o ON tcl.trigger_id=o.trigger_id INNER JOIN master_trigger_conditions ON t.trigger_condition_id = master_trigger_conditions.trigger_condition_id INNER JOIN master_product_type ON master_product_type.product_type_id = o.product_type INNER JOIN master_buy_sell ON master_buy_sell.buy_sell_id = o.buy_sell WHERE t.user_id =1 and is_executed = False")
    rows = cursor.fetchall()
    open_group_order = []
    for row in rows:
    ...
    cursor.execute("SELECT t.created_on, t.trigger_name,t.symbol, master_trigger_conditions.trigger_condition, master_buy_sell.buy_sell,master_product_type.product_type,o.trigger_id,o.order_id,o.amount FROM triggers t JOIN public.trigger_condition_list tcl ON trigger_list like '%' || t.trigger_id || '%' JOIN public.group_order o ON tcl.trigger_id=o.trigger_id INNER JOIN master_trigger_conditions ON t.trigger_condition_id = master_trigger_conditions.trigger_condition_id INNER JOIN master_product_type ON master_product_type.product_type_id = o.product_type INNER JOIN master_buy_sell ON master_buy_sell.buy_sell_id = o.buy_sell WHERE t.user_id =1 and is_executed = True")
    rows2 = cursor.fetchall()
    ...
    print("open_group_order ",open_group_order)     
    cursor.execute("SELECT username FROM current_login_user")
    current_user = cursor.fetchone()
    current_user = current_user[0]
    print('current_user :',current_user)
...
def cancel_order(request):
    if request.method=="POST":
        ...
        is_cancelled = cursor.fetchall()
            # triggers = cursor.fetchall()
        print("is_cancelled :===",is_cancelled[0])
        print("is_cancelled :===",is_cancelled[0][0])
        print("")
        if is_cancelled[0][0] == False:
            print("inside if")
            print("is_cancelled False and now true")
            ...
            connection.commit()
            print("is_cancelled True and now False")
            return Response({"status":"success", "message": "success"})
    # return Response({"status":"error", "message": "fail"})
